...
Next, enable Project Types in Kotlin DSL scripts by setting the `org.gradle.kotlin.dsl.dcl=true` flag in the root project's `gradle.properties` file.
This flag allows Gradle to understand Project Types used in `*.kts` buildscripts alongside imperative project configuration logic.

Finally, identify which declarative _Ecosystem Plugins_  are needed to expose the Project Types relevant to your project. 
...
For more information, see the [Project Features reference](https://declarative.gradle.org/docs/reference/project-features/) in the Declarative Gradle documentation.
...
Start with the simplest projects that can be nicely described by existing declarative Project Types, such as [Gradle's prototype ecosystem plugins](https://github.com/gradle/declarative-gradle/tree/main/unified-prototype).

Project Types can be used in Kotlin DSL buildscripts once the appropriate Ecosystem Plugin is applied in your settings file. 
You can add a new block to the bottom of your `build.gradle.kts` file using the bound name and definition object of the Project Type you will be using.
For example:
...
Depending on the defaults and conventions implemented in this Project Type, your project's configuration, and how they may conflict, your project may or may not continue to be buildable at this point.

Once the appropriate Ecosystem Plugin is applied in your settings, begin migrating configuration into the Project Type’s extension.
For example, any `dependencies` declared by your project in Gradle’s top-level `dependencies` block be moved to the equivalent location provided by your Project Type’s extension.

Fully configure the Project Type by: 
- Adding all dependencies
- Setting version information
- Providing necessary directory values
- Assigning any other property values needed

You may need to comment out or relocate any features that are not currently supported by the DCL or the Project Type in use.
...
Once all configuration has been moved into the Project Type and the project builds successfully:
...
As you migrate more complex projects, you may find that existing Project Types do not cover all your configuration needs. 
It may be necessary to create custom Project Types to support advanced or project-specific features.
...
Use this included build to define and test custom declarative plugins, including new Project Types tailored to your needs that can be used by the build you are migrating. 
...
This Declarative Gradle plugin allows you to create new Project Types in Java.

See the [Project Features reference](https://declarative.gradle.org/docs/reference/project-features/) for more information on how to implement Project Types.
...
A Project Type plugin requires at least 5 parts:
1. A **top-level DSL interface** that will be configured in the DCL file by any project using this Project Type
2. A **Project Type plugin** that implements `Plugin<Project>` and has a `@RegistersProjectTypes` annotation referencing a `ProjectTypeBinding` class.
3. A **Project type binding** class that implements a `ProjectTypeBinding` tying together the user-configurable definition, its build model, and a DSL element to bind the definition to in the DSL.
4. An **"Ecosystem" plugin** that implements `Plugin<Settings>` and registers the project type via `@RegistersSoftwareTypes(MySoftwareTypePlugin.class)`
5. **Registration of the new plugins** in the `build.gradle.dcl` file of the project that contains them using the `registers` block provided by the `javaGradlePlugin` Project Type's top-level extension 
...
Once you've created a custom Project Type, you're ready to apply it to more complex projects.

Start by:
- Adding your new Ecosystem Plugin to the list of plugins in your `settings.gradle.dcl` file
- Adding your new Project Type block to your project's existing `build.gradle.kts` file
- Configuring the new Project Type in the same way as you did with your simpler projects using pre-existing prototype Declarative plugins, by moving things like version and dependency declarations from the imperative parts of your buildscript, to the declarative part inside of your new Project Type

This process is iterative. 
You’ll migrate a piece of configuration, then realize the Project Type is missing some capability. 
...
Before migrating any configuration, you should verify that your custom Project Type is correctly wired into the build:
...
You should save your project before attempting to migrate larger bits of configuration, such as the application of each 3rd party plugin, into your Declarative Project Type plugin’s logic.

Once all logic has been moved into your Project Type and the imperative part of the build script is empty:
- Rename the file to `build.gradle.dcl`
- Verify your build one last time

### 8. Continue until all projects are using DCL

Once you’ve successfully set up your included build and applied your first custom Project Type to a project, you can continue migrating the remaining projects that require customization using the same approach.

Repeat the process—refining or extending your Project Types as needed—until every project in your build uses a declarative build script (`*.dcl` file).
...
If you find that certain properties are always set to the same value across multiple projects using the same Project Types (for example, setting the same JDK version in every library project), you can configure this just once in `defaults` rather than repeating it in each build script.
...
These are supported within Kotlin DSL (`*.kts`) buildscripts using Project Types but **not** within fully declarative `*.dcl` files.
...
When configuring Project Types, you may find yourself needing to use explicit empty pairs of braces (`{}`), especially when adding new elements to `NamedDomainObjectContainer`s, even if it is not necessary to perform any configuration on these new elements. 
