...
In February and March 2025, using the version of Declarative Gradle available as EAP 3, we migrated the [`gradle client`](https://github.com/gradle/gradle-client) application to Declarative Gradle. This migration included defining a custom Software Type to represent the plugins and tools used by one of its projects and replacing all Kotlin buildscripts with DCL files.

This migration gave us the opportunity to explore the DCL migration process in depth and identify common stumbling blocks and areas for improvement.
...
For your own migrations, we recommend starting with simpler projects—those that closely resemble our declarative samples. These are more likely to work with minimal effort and help build confidence before tackling complex parts of your build.
## Preparing for Migration

For the migration of the Gradle-Client project and to support the latest Declarative Gradle (DCL), we eventually upgraded the Gradle wrapper to version [gradle-8.14-milestone-4](https://services.gradle.org/distributions/gradle-8.14-milestone-4-bin.zip).
...
As the `:gradle-client` project is a Kotlin Multiplatform project (which actually only targets the JVM), we applied the corresponding Ecosystem Plugin in the root project's existing `settings.gradle.kts` file:
...
This is likely to change in a future release, but as it is “merely” a performance optimization, it can be omitted without impacting build correctness.
- `repositoriesMode = RepositoriesMode.FAIL_ON_PROJECT_REPOS` becomes `repositoriesMode = FAIL_ON_PROJECT_REPOS`, as Gradle automatically understands enum types used in assignments this way.
...
Inside each individual project’s buildscript, you can incrementally introduce declarative Software Types alongside imperative logic.
...
Core functionality, such as typical `implementation`, `API`, and test dependencies for simple library projects in common ecosystems, is already supported by the existing Software Types provided by Gradle's prototype plugins.
...
The DCL version is more concise and clearly communicates the same intent..
...
As a result, we could not simply apply the existing `kotlinApplication` Software Type.
Like others in the prototype set, that plugin does not yet support composition or extensibility, features that would allow combining functionality like `kotlinApplication` with SQLDelight or Detekt in a modular way.
...
The approach we used is the one outlined in our [Migration Guide](migration-guide.md): 
...
This is because `buildSrc` is built after the project's settings script is evaluated, so it can't contain a `Plugin<Settings>` that is used by the build.
Our Ecosystem Plugin will be implemented as a settings plugin.

We created a new `/build-logic` directory and added the following to the root `settings.gradle.kts`:
...
Next, we created a `/build-logic/plugins` subproject in `/build-logic` that contains our new Software Types.  
...
We could have registered the plugins directly in the root project of the `/build-logic` build, this extra level of separation could be useful if you have other imperative logic to migrate that wouldn’t be entirely contained in your new Software Types.

It allows for:
Creating other plugins, such as settings plugins to provide additional task types or global configuration that you could then apply to your build in distinct projects.
...
An initial idea for implementing our custom Software Type might be to extend an existing one like `kotlinApplication`, to take advantage of support for dependencies and SDK versioning already implemented in its plugin. 
...
/CustomDesktopComposeApplicationPlugin.java`looks like this:
...
To support this, we needed to define a model interface (i.e., the extension type) that backs the Software Type. 
This interface defines the configuration DSL that consumers will use declaratively.

In `build-logic/plugins/src/main/java/org/gradle/client/softwaretype/CustomDesktopComposeApplication.java` we start with an empty interface:
...
At this point, the plugin is usable in the root `gradle-client` build.
### Writing a New Ecosystem Plugin

In Declarative Gradle, you don’t apply plugins in project .dcl files.
Why?
Because plugin application is imperative.

Telling Gradle to _apply_ a plugin is _build logic_ dealing with _how_ a build behaves, instead of _what_ a project represents.
...
It’s important to keep in mind that—if our vision for Declarative Gradle succeeds—writing your own Ecosystem Plugins won’t be something you’ll need to do for every project.
...
This is often the best place to begin—dependencies are foundational to any build, and it's relatively easy to verify correctness.
...
That returns a `LibraryDependencies` instance defined in [LibraryDependencies.java]( https://github.com/gradle/declarative-gradle/tree/v0.1.41/unified-prototype/unified-plugin/plugin-common/src/main/java/org/gradle/api/experimental/common/LibraryDependencies.java):
...
Which extends `BasicDependencies` defined in [BasicDependencies.java](https://github.com/gradle/declarative-gradle/tree/v0.1.41/unified-prototype/unified-plugin/plugin-common/src/main/java/org/gradle/api/experimental/common/BasicDependencies.java):
...
Provides one or more getters that return `org.gradle.api.artifacts.dsl.DependencyCollector`.
...
This allows method calls like `implementation(String)` or `api(Project)` to map correctly to overloads of `DependencyCollector.add(...)` present on the collectors returned by those getters.
...
That’s because `DependencyCollector` does exactly what it says: it collects dependency declarations. It does not resolve them or apply them to a project. 
...
For every collector, we must find the appropriate resolvable `Configuration` and use `addAllLater` to lazily add the collected dependencies from that collector to it during dependency resolution.

Once this wiring is in place, you can move dependency declarations from the imperative top-level `dependencies` block to the corresponding collector in our Software Type’s new nested `dependencies` block.
...
Then you can reuse these types for your custom Software Type:
...
and _removing_ those same dependencies from the old `jvmMain.dependencies` block where, we should end up with a fully working build.

This makes sense, as the `:gradle-client` project was a KMP project that only targets the JVM platform. 
Swapping out the declarations in `jvmMain.dependencies` for the `jvm.dependencies` block yields the same result, as the Software Type handles wiring dependencies into the correct configurations.

The JDK version declaration is also required by this Software Type. 
...
We already have a working  `kotlinApplication` and extend it with custom functionality (like SQL Delight and Detekt).
...
This gives us access to all the existing functionality already defined in that Software Type—dependencies, target configurations, nested blocks, and more.
...
We needed to wire it up so that it behaved like the original `kotlinApplication` type. 
That logic lived inside the `StandaloneKmpApplicationPlugin`’s apply method. To reuse it, we first had to refactor that logic into a separate utility method.

We created a `PluginWiring` class with a public statc `wirePlugin` method (later renamed to `wireKMPApplication`). 
This allowed us to delegate setup of the nested type directly from our plugin.
...
After making these changes, we were able to move most of the “basic” logic of our KMP application to the plugin using the nested `kotlinApplication` block.
...
But for now, with EAP3—this brute-force composition approach gets us the reuse we need, without duplicating the logic we’ve already built.
### Wiring Additional Functionality Party Plugins
...
To address this, we gradually build support for each feature within our declarative desktopComposeApp Software Type. 
...
We need to create a new nested block in our Software Type to hold configuration data for that plugin.
We need to apply the plugin wiring that configuration data as needed.
...
To keep our code organized, we followed the pattern used for wiring the KMP Application support and created a new dedicated support class `DetektSupport`. [Here’s the `wireDetekt` method](https://github.com/gradle/gradle-client/blob/e0b98007a8a1b337729b2df3fb40c80a0fa59e0e/build-logic/plugins/src/main/java/org/gradle/client/softwaretype/detekt/DetektSupport.java):
...
To work around this, we need to retrieve the value of our lazy `Property` via `get()` and call `setParallel()` using the result—but that can only be done once the configuration model has been populated.
...
The code configuring the plugin will not be able to eagerly access the final values in the model’s Java bean properties and plain collections at that point and will have to wait until later for them to be present, and using `afterEvaluate` delays our access to these until Gradle is ready to provide them.
...
To replicate this structure in our DCL-based Software Type, we model the databases block using a `NDOC`., This allows us to preserve much of the original DSL structure, now nesting a `sqlDelight` block inside our `desktopComposeApp` top-level block:
...
This can be customized, but is sufficient for our needs.
We kept the name `database` instead of using `create`, because it sounds more declarative and aligns better with the style we’re aiming for than a method called `create`, which has decidedly imperative conotations.
...
Since the SQLDelight plugin uses lazy Properties, we call `.set()` and supply our extensions configurations _without_ calling `.get()` to realize their values.
...
Build new DSL extensions to hold the configuration data needed by the plugin
Move the configuration from the imperative part of our buildscript into the new blocks using the property types
Write code to be called by our plugin during application to wire values from our DCL extension into the (Compose) plugin’s extension in an `afterEvaluate` closure
...
If you’re looking at this code and thinking it’s more complex than it should be—don’t worry, we feel the same. Once map support lands in a future EAP, we plan to revisit this and simplify the structure considerably.
...
Behind the scenes, this isn’t modeled as a container of user-defined build types (like you might see with NDOCs elsewhere). Instead, it's implemented using several static nested extensions.
...
## Conclusion
There's no “easy button”, but as this case study shows, it's not _that_ hard to start using DCL in real-world project.
...
Once your included build is fully set up, development is smooth and iterative.
